...
#[AsCommand(
    name: 'create:leap-month-list',
    description: 'Tạo danh sách tháng nhuận của các năm Âm lịch',
    hidden: false,
    aliases: []
)]

class CreateLeapMonthList extends Command
{
    ...
    protected function renderDataTable(InputInterface $input, OutputInterface $output): void
    {
        ...
        $table->setHeaderTitle('Danh sách tháng nhuận Âm lịch');
        $table->setHeaders([
            '#',
            'Năm Âm lịch',
            'Tháng nhuận',
    ...
    {
        $content = <<<HELP
        Lệnh tạo ra một danh sách các điểm Sóc của tháng 11 tương ứng với năm Âm lịch.
        HELP;
    ...
    protected function getList(InputInterface $input): array
    {
        ...
        do {
            ...
            if ($leap->isLeap()) {
                $outputDate = $this->jdToDateTime($leap->getMidnightJd(), $timezone);

                array_push($data, [
                    'lunar_year' => $year,
                    'leap_month' => $leap->getMonth(),
