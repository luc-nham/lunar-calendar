...
#[AsCommand(
    name: 'create:new-moon-list',
    description: 'Tạo danh sách điểm Sóc của các tháng trong 1 năm Âm lịch',
    hidden: false,
    aliases: []
)]

class CreateNewMoonList extends Command
{
    ...
    protected function renderDataTable(InputInterface $input, OutputInterface $output): void
    {
        ...
        $table->setHeaderTitle('Danh sách điểm sóc của các tháng trong năm Âm lịch');
        $table->setHeaders([
            '#',
            'Năm Âm lịch',
            'Tháng Âm lịch',
    ...
    {
        $content = <<<HELP
        Lệnh tạo ra một danh sách các điểm Sóc của các tháng trong năm Âm lịch.
        HELP;
    ...
    protected function getList(InputInterface $input): array
    {
        ...
        do {
            for ($month = 1; $month <= 12; $month ++) {
                ...
                $leapMonth = $corrector->getLeapMonth();

                $outputDate = $this->jdToDateTime($newMoon->getMidnightJd(), $timezone);

                array_push($data, [
                    'year' => (string)$year,
                    'month' => (string)$month,
                    'new_moon'  => $outputDate->format($input->getOption('date-format')),
                    'timestamp' => $outputDate->getTimestamp(),
                    'jd' => $newMoon->getMidnightJd()
                ]);

                if ($leapMonth && $leapMonth->getMonth() == $month) 
                {
                    $outputDate = $this->jdToDateTime($leapMonth->getMidnightJd(), $timezone);
