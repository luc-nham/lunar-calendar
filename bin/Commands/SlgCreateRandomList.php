...
use VanTran\LunarCalendar\Sunlongitude\DateTimeToSunlongitude;
use VanTran\LunarCalendarCli\Traits\JulianToUnix;

#[AsCommand(
    name: 'slg:create-random-list',
    description: 'Tạo danh sách ngẫu nhiên các điểm mốc Kinh độ Mặt trời',
    hidden: false,
    aliases: []
)]

class SlgCreateRandomList extends Command
{
    use JulianToUnix;
    ...
    protected function renderDataTable(InputInterface $input, OutputInterface $output): void
    {
        ...
        $table->setHeaderTitle('Danh sách điểm mốc KDMT');
        $table->setHeaders([
            '#',
            'Ngày',
            'Giờ',
    ...
    protected function getDataList(InputInterface $input): array
    {
        ...
        for ($i = 1; $i <= $input->getOption('quantity'); $i ++) 
        {
            ...
            $sl = new DateTimeToSunlongitude($date);

            array_push($data, [
                'date' => $date->format('Y-m-d'),
                'time' => $date->format('H:i:s'),
                'timezone' => $timezone->getName(),
                'degrees' => $sl->getDegrees(true),
                'jd' => $sl->getJd(),
                'timestamp' => $this->jdToUnix($sl->getJd())
            ]);
        }
